...
pub fn trackingCameraSystem(allocator: std.mem.Allocator, entity_to_track: engine.ecs.Entity) game.Ecs.System {
    ...
    return game.Ecs.System{
        ...
        .runFn = struct {
            ...
            fn orbitTransformation(camera: *rl.Camera3D, transform: rl.Matrix) void {
            ...
            fn run(results: []game.Ecs.QueryResult, myecs: *game.Ecs, st: *game.state.GameState) void {
                ...
                orbitTransformation(camera, transform);

                const body = st.physics.?.world.getBody(body_id.*);
                if (!body.isActive()) {
                    ...
                    std.log.warn(
                        ...
                        \\
                    , .{face_up});
                    st.upward_face = game.dice.DieType.six.faceIdxToValue(face_up - 1);
                    ...
                    const is_hit = st.physics.?.world.rayTestClosest(
                        &ray_from,
                        &ray_to,
                        .{ .default = true },
